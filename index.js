...
const { getAttendanceReport, cleanup } = require("./attendance");
...
function formatAttendanceReport(data) {
    ...
    if (data.total_classes === 0) {
        ...
        lines.push(
            ...
            "",
            `• 📈 <b>Percentage:</b> ${data.overall_percentage}% ${statusIcon}`,
            "",
            `📌 <b>Status:</b> ${statusText}`
        );

        if (data.attendance_status.above_threshold && data.skippable_hours > 0) {
            lines.push(`🛑 <b>Skippable:</b> You can miss <u><b>${data.skippable_hours}</b></u> classes.`);
        } else if (!data.attendance_status.above_threshold) {
            lines.push(`📚 <b>Required:</b> Attend <u>${data.attendance_status.required_hours}</u> more classes to reach 75%.`);
        }
    }
...
fastify.post("/attendance", async request => {
